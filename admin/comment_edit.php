...
<div class="col-10 pt-3 pb-5">
  ...
  <form action="traitement_edit_comment.php" method="POST" class="py-3 pl-5">
     <h5>Commentaire envoyé par : <?= $res->firstname; ?> <?= $res->lastname; ?>, le <?= $created; ?></h5>
      <div class="form-row py-3">
      <textarea name="com" id="com" class="px-1 w-50" cols="30" rows="10"><?= $res->commentaire.'<br>----------<br><span class="text-danger">Raison de la modération: </span>'; ?></textarea>
    </div>
    <input type="hidden" name="moderate" value="yes">
    <input type="hidden" name="id" value="<?= $res->id; ?>">
    <button type="submit" class="btn btn-dark my-2 w-25 margin-auto">Mettre à jour</button>
  </form>
</div>
